	...
	return encoded.map(obj => {
		...
		for (let key in obj) {
			if (key.endsWith('Id') && !(['groupId', 'service', 'formatSize', 'ca'].includes(key))) {
				let decodedKey = key.slice(0, -2);
				decodedId[decodedKey] = translations[obj[key]] || obj[key];
				uniq.add(obj[key]);
			}
			else {
				decodedId[key] = obj[key];
			}
		}
		return decodedId;
	});
}
function uniqId(encoded, translations) {
	let uniq = new Set();
	return encoded.map(obj => {
		for (let key in obj) {
			if (key.endsWith('Id') && !(['groupId', 'service', 'formatSize', 'ca'].includes(key))) {
...
const uniq = uniqId(encoded, translations);
console.log(decoded, uniq);
//decoded.uniq);
